...
# Start vtund as server with tunnel2.server as the config file
execute "Start vtund as server" do
  command "vtund -s -f '/tmp/tunnel2.server'"
end

# Start vtund as client and connect to secgw via tunnel1
execute "Start vtund as client" do
  command "vtund -f '/tmp/tunnel1.client' tunnel1 #{node[:opsworks][:layers][:secgw][:instances].first[1][:private_ip]}"
end
